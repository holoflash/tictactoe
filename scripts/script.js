import { firstBest, anyEmpty } from './squareChoice.js';
...
const ai1 = (player) => {
    ...
    setTimeout(() => {
        //Use the imported square selection function firstBest()
        firstBest(player);
        winCheck(player);
        player1turn = !player1turn;
...
const ai2 = (player) => {
    ...
    if (player !== (player1turn ? player1 : player2) || !gameInProgress) return;
    setTimeout(() => {
        //Use the imported square selection function anyEmpty()
        anyEmpty(player);
        winCheck(player);
        player1turn = !player1turn;
        player = player1turn ? player1 : player2;
...
function winCheck(player) {
    ...
    if (
        gameBoard.every((row, index) => row[index] === player.mark) ||
        gameBoard.every(
            (row, index) => row[gameBoard.length - 1 - index] === player.mark
        )
    ) {
        RESULT.innerText = `${player.mark} WINS!`;
        gameInProgress = false;
