(() => {
    ...
    const human = (player) => {
        ...
        const handleClick = (event) => {
            if (event.target.innerText !== '' || gameInProgress === false)
                return;
    ...
    const ai1 = (player) => {
        ...
        if (player !== (player1turn ? player1 : player2) || !gameInProgress)
            return;
        setTimeout(() => {
            let placed = false;
    ...
    function winCheck(player) {
        if (
            gameBoard.some((row) => row.every((cell) => cell === player.mark))
        ) {
        ...
        if (
            ...
            gameBoard.every(
                (row, index) =>
                    row[gameBoard.length - 1 - index] === player.mark
            )
        ) {
            RESULT.innerText = `${player.mark} WINS!`;
            gameInProgress = false;
